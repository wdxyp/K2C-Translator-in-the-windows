...
class TranslationDataset(Dataset):
    def __init__(self, zh_tokens, ko_tokens, zh_vocab, ko_vocab):
    ...
    def __getitem__(self, idx):
        ...
        return torch.LongTensor(zh_idx), torch.LongTensor(ko_idx)

def collate_fn(batch):
    zh_batch, ko_batch = zip(*batch)
    ...
    return zh_padded, torch.LongTensor(zh_lens), ko_padded, torch.LongTensor(ko_lens)
...
def train_model(train_loader, test_loader, zh_vocab, ko_vocab, device):
    INPUT_DIM = len(zh_vocab)
    OUTPUT_DIM = len(ko_vocab)
    ENC_EMB_DIM = 256
    DEC_EMB_DIM = 256
    HID_DIM = 512
    N_LAYERS = 1
    ...
    model_path = os.path.join(model_folder, 'best_model_zh2ko_attn.pth')
    ...
    for epoch in range(50):
        ...
        if avg_test_loss < best_test_loss:
            ...
            no_improve = 0
            torch.save(model.state_dict(), os.path.join(model_folder, 'best_model_zh2ko_attn.pth'))
            print("  -> 测试损失下降，保存最佳模型！")
        else:
            no_improve += 1
    ...
    torch.save(model.state_dict(), os.path.join(model_folder, f'model_zh2ko_{timestamp}.pth'))
    with open(os.path.join(model_folder, f'zh_vocab_zh2ko_{timestamp}.pkl'), 'wb') as f: pickle.dump(zh_vocab, f)
    with open(os.path.join(model_folder, f'ko_vocab_zh2ko_{timestamp}.pkl'), 'wb') as f: pickle.dump(ko_vocab, f)
    print("训练全过程完成！")
...
def start_training():
    ...
    zh_train, zh_test, ko_train, ko_test = train_test_split(zh_tokens, ko_tokens, test_size=0.1, random_state=42)

    train_ds = TranslationDataset(zh_train, ko_train, zh_vocab, ko_vocab)
    test_ds = TranslationDataset(zh_test, ko_test, zh_vocab, ko_vocab)
    ...
    train_model(train_loader, test_loader, zh_vocab, ko_vocab, device)
...
if __name__ == "__main__":
    root = tk.Tk()
    root.title("中韩翻译训练 V1.0 (Attention)")
    corpus_file_paths = []
    ...
    btn_train = tk.Button(root, text="2. 开始训练 (中文→韩文)", command=start_training, bg='green', fg='white')
