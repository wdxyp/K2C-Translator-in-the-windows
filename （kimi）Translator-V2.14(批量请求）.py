...
KIMI_MODEL = os.getenv('KIMI_MODEL') or 'kimi-k2.6'
...
_kimi_min_interval = 0.5
_translation_cache = {}  # 内存缓存：记录已翻译过的文本，避免重复请求 API
_kimi_max_workers = 10  # Tier1 支持 50 并发，这里保守设置 10
...
def _kimi_post(payload, timeout):
    # 并发时不应使用全局 _kimi_request_lock，改为各线程竞争 _kimi_rate_lock 即可
    _kimi_throttle()
    headers = {
                                ...
                                if choices:
                                    ...
                                    if content is not None:
                                        ...
                                        if strict_no_hangul and (translated == original_stripped or HANGUL_RE.search(translated)):
                                            retries += 1
                                            continue
                                        _translation_cache[cache_key] = translated  # 3. 写入缓存
                                        return translated
                    except Exception:
                        pass
                retries += 1
...
def _fallback_translate_one(text, from_lang, to_lang):
    ...
    if not text or not text.strip():
        return text
    if "\n" not in text:
        return kimi_translate(text, from_lang, to_lang)
    lines = text.split("\n")
    out_lines = []
    for line in lines:
        if line.strip():
            out_lines.append(kimi_translate(line, from_lang, to_lang))
        else:
            out_lines.append(line)
    return "\n".join(out_lines)

def _extract_json_fragment(s):
    if s is None:
        return None
    t = str(s).strip()
    if not t:
...
def _kimi_translate_batch_once(text_list, from_lang, to_lang, temperature=1):
    if not text_list:
        return []
    if not KIMI_API_KEY or KIMI_API_KEY == "YOUR_KIMI_API_KEY":
    ...
    for idx, text in enumerate(text_list):
        ...
        items.append({"id": str(idx), "lines": lines})
        input_line_counts.append(len(lines))

    strict_no_hangul_flags = []
    for text in text_list:
        original_stripped = str(text or "").strip()
        strict_no_hangul = (from_lang == "kor") and (to_lang != "kor") and bool(HANGUL_RE.search(original_stripped))
        strict_no_hangul_flags.append(strict_no_hangul)

    user_prompt = (
        f"Translate from {from_lang} to {to_lang}.\n"
        "Return ONLY valid JSON.\n"
        "Output format: a JSON array.\n"
        "Each element must be an object: {\"id\": \"<id>\", \"lines\": [\"...\"]}.\n"
        "Rules:\n"
        "1) Keep every id unchanged.\n"
        "2) Do NOT add or remove elements.\n"
        "3) For each element, keep lines array length EXACTLY the same as the input.\n"
        "4) Translate each non-empty line. Keep empty lines as empty strings.\n"
        "5) Do NOT add explanations, notes, pinyin, quotes, markdown, or any extra text.\n"
        "Input JSON array:\n"
    ...
    _mark_api("start(batch)")
    response = _kimi_post(payload, timeout=120)
    response.encoding = "utf-8"
    if response.status_code != 200:
        _mark_api(f"http={response.status_code}(batch)")
        err_msg = f"KIMI batch http={response.status_code}"
        try:
            body = response.json() or {}
            err_detail = body.get("error") or {}
            msg = str(err_detail.get("message") or "")
            if msg:
                err_msg += f" - {msg}"
                print(f"[警告] API 报错详情: {msg}")
        except Exception:
            pass
        raise RuntimeError(err_msg)

    _mark_api("http=200(batch)")
    result = response.json() or {}
    choices = result.get("choices") or []
    if not choices:
        raise RuntimeError("KIMI batch empty choices")
    message = (choices[0] or {}).get("message") or {}
    content = message.get("content")
    if content is None:
        raise RuntimeError("KIMI batch empty content")
    ...
    if not isinstance(parsed, list):
        raise RuntimeError("KIMI batch invalid json")
    ...
    for obj in parsed:
        ...
        if not isinstance(lines, list):
            ...
            if txt is None:
                continue
            lines = _normalize_text_newlines(txt).split("\n")
        out_map[str(oid)] = [("" if v is None else str(v)) for v in list(lines)]

    outputs = []
    for idx in range(len(text_list)):
        key = str(idx)
        if key not in out_map:
            raise RuntimeError("KIMI batch missing id")
        out_lines = out_map[key]
        if len(out_lines) != input_line_counts[idx]:
            raise RuntimeError("KIMI batch line count mismatch")
        translated = "\n".join(out_lines).strip()
        if strict_no_hangul_flags[idx]:
            original_stripped = str(text_list[idx] or "").strip()
            if translated == original_stripped or HANGUL_RE.search(translated):
                translated = _fallback_translate_one(text_list[idx], from_lang, to_lang)
        outputs.append(translated)
    return outputs

def _kimi_translate_batch_recursive(text_list, from_lang, to_lang):
    if not text_list:
        return []
    if len(text_list) == 1:
        return [_fallback_translate_one(text_list[0], from_lang, to_lang)]

    temperature = 1
    try:
        temperature = _env_float("KIMI_TEMPERATURE", 1)
    except Exception:
        temperature = 1

    try:
        return _kimi_translate_batch_once(text_list, from_lang, to_lang, temperature=temperature)
    except Exception:
        mid = len(text_list) // 2
        left = _kimi_translate_batch_recursive(text_list[:mid], from_lang, to_lang)
        right = _kimi_translate_batch_recursive(text_list[mid:], from_lang, to_lang)
        return left + right

def kimi_translate_batch(text_list, from_lang, to_lang):
    if not text_list:
        return []
    ...
    # 优化：减小批次大小以降低单次请求耗时，配合并发提升整体吞吐
    batch_size = max(1, _env_int("KIMI_BATCH_SIZE", 20)) 
    batch_char_limit = max(200, _env_int("KIMI_BATCH_CHAR_LIMIT", 10000))
    ...
    while start < len(pending_texts):
        ...
        chunk_indices = []
        char_sum = 0
        while start < len(pending_texts) and len(chunk_texts) < batch_size:
            t = pending_texts[start]
            projected = char_sum + len(t)
            if chunk_texts and projected > batch_char_limit:
                break
            chunk_texts.append(t)
            chunk_indices.append(pending_indices[start])
            char_sum = projected
            start += 1
        all_chunks.append((chunk_texts, chunk_indices))

    total_chunks = len(all_chunks)
    print(f"[系统] 准备处理 {total_chunks} 个翻译批次 (并发数: {_kimi_max_workers})...")

    # 2. 并发执行批次
    def process_chunk(chunk_data, chunk_idx):
        chunk_texts, chunk_indices = chunk_data
        msg = f"正在请求批次 {chunk_idx + 1}/{total_chunks} ({len(chunk_texts)} 条文本)..."
        print(f"[进度] {msg}")
        update_ui_status(msg) # 这里会标记进度，防止心跳误报
        try:
            translated_chunk = _kimi_translate_batch_recursive(chunk_texts, from_lang, to_lang)
            return chunk_indices, translated_chunk
        except Exception as e:
            print(f"[错误] 批次 {chunk_idx + 1} 最终失败: {e}")
            return chunk_indices, [None] * len(chunk_texts)

    with ThreadPoolExecutor(max_workers=_kimi_max_workers) as executor:
        future_to_idx = {executor.submit(process_chunk, chunk, i): i for i, chunk in enumerate(all_chunks)}
        for future in as_completed(future_to_idx):
            chunk_indices, translated_chunk = future.result()
...
def get_translation_batch(text_list):
    ...
    translated_list = kimi_translate_batch(text_list, from_lang, to_lang)
    out = []
...
def run_translation_task(input_file, output_folder):
    ...
    try:
        ...
        output_file = os.path.join(output_folder, (custom_name if custom_name else f"translated_v2.14_{os.path.basename(input_file).split('.')[0]}") + file_ext)
...
def save_to_corpus(orig, trans):
    if generate_corpus.get() and orig:
        ...
        try:
            ...
            direction = "unknown"
        corpus_file = f'Corpus/Corpus_v2.14_{direction}_{timestamp}.xlsx'
        to_lang = TO_LANG_MAP.get(direction)
        seen = set()
        filtered_orig = []
        filtered_trans = []
...
ttk.Label(main_frame, text="KIMI 翻译(批量请求) & 自动校准系统", style="Header.TLabel").pack(pady=(0, 20))
